...
REPO_ROOT = Path(os.environ.get("WEWRITE_RESOURCE_DIR") or Path(__file__).resolve().parent.parent)
...
def is_campus_party_template(template_name: str) -> bool:
    return template_name == "campus-party"
...
def render_image_block(image: dict[str, Any] | None, *, template_name: str = "") -> str:
    ...
    if is_studio_brief_template(template_name):
        return (
            ...
            f'<p style="font-size: 11px; color: #7f766b; text-align: left; margin: 0; line-height: 1.6;">{caption}</p>'
    ...
    if is_campus_party_template(template_name):
        return (
            '<section style="background: #ffd897; padding: 8px 16px 16px; text-align: center;">'
            '<section style="background: #fffbea; padding: 6px 8px; border-radius: 8px; border: 1px solid #f3c56d;">'
            '<section style="display: flex; justify-content: space-between; margin-bottom: 5px;">'
            + "".join('<section style="width: 10px; height: 16px; background: #ffffff; border-radius: 3px;"></section>' for _ in range(10))
            + "</section>"
            f'<img src="{url}" style="width: 100%; display: block; margin: 0; border-radius: 6px;" />'
            '<section style="display: flex; justify-content: space-between; margin-top: 5px;">'
            + "".join('<section style="width: 10px; height: 16px; background: #ffffff; border-radius: 3px;"></section>' for _ in range(10))
            + "</section>"
            f'<p style="font-size: 12px; color: #9a4a1e; text-align: center; margin: 8px 0 0; line-height: 1.6;">{caption}</p>'
            "</section></section>"
        )
    return (
...
def render_section_heading(section: dict[str, Any], *, template_name: str = "") -> str:
    ...
    if is_campus_party_template(template_name):
        return (
            '<section style="background: #ffd897; padding: 20px 16px 8px; text-align: center;">'
            '<section style="display: inline-block; padding: 0 6px 4px; background: #ffc000;">'
            '<section style="background: #ffffff; padding: 7px 16px; border-radius: 2px;">'
            f'<p style="font-size: 12px; color: #c53b22; letter-spacing: 2px; text-transform: uppercase; margin: 0 0 4px; font-weight: 700;">{section_en}</p>'
            f'<p style="font-size: 19px; font-weight: 800; color: #e60012; line-height: 1.35; margin: 0;">{section_cn}</p>'
            "</section></section></section>"
        )
    return (
...
def render_body_paragraphs(value: Any, *, margin_bottom: str = "8px", template_name: str = "") -> str:
    ...
    elif is_campus_party_template(template_name):
        font_size, text_color, line_height = "15px", "#333333", "2"
    else:
...
def render_card_block(block: dict[str, Any], *, highlight: bool, template_name: str = "") -> str:
    ...
    if is_campus_party_template(template_name):
        number_bg = "#e60012" if highlight else "#ffc000"
        number_fg = "#ffffff" if highlight else "#c50000"
        return (
            '<section style="background: #ffd897; padding: 0 16px 14px;">'
            '<section style="background: #ffffff; border-radius: 10px; padding: 16px 16px 14px; border: 1px solid #cd5029;">'
            '<section style="display: flex; align-items: flex-start;">'
            f'<section style="min-width: 34px; width: 34px; height: 34px; background: {number_bg}; color: {number_fg}; '
            'font-size: 13px; font-weight: 800; line-height: 34px; text-align: center; border-radius: 50%; '
            'margin-right: 12px; flex-shrink: 0;">'
            f"{number}</section>"
            '<section style="flex: 1;">'
            f'<p style="font-size: 17px; font-weight: 800; color: #c50000; margin: 0 0 8px; line-height: 1.45;">{title}</p>'
            f"{body}"
            f'<p style="font-size: 12px; color: #9a4a1e; margin: 8px 0 0; letter-spacing: 0.5px;">{source}</p>'
            "</section></section></section></section>"
        )
    return (
...
def render_quote_block(block: dict[str, Any], *, template_name: str = "") -> str:
    ...
    if is_campus_party_template(template_name):
        return (
            '<section style="background: #ffd897; padding: 0 16px 14px;">'
            '<section style="background: #fffbea; border-left: 5px solid #e60012; border-right: 1px solid #cd5029; border-top: 1px solid #cd5029; border-bottom: 1px solid #cd5029; padding: 14px 16px;">'
            f'<p style="font-size: 15px; color: #333333; line-height: 2; margin: 0 0 8px;">{text}</p>'
            f'<p style="font-size: 12px; color: #9a4a1e; margin: 0; text-align: right;">{attribution}</p>'
            "</section></section>"
        )
    return (
...
def render_takeaways_block(block: dict[str, Any], *, template_name: str = "") -> str:
    ...
    if is_campus_party_template(template_name):
        return (
            '<section style="background: #ffd897; padding: 0 16px 14px;">'
            '<section style="background: #ffffff; border-radius: 10px; border: 1px solid #cd5029; padding: 16px 16px 8px;">'
            f'<p style="font-size: 16px; font-weight: 800; color: #e60012; margin: 0 0 10px; text-align: center;">{title}</p>'
            f'<ul style="margin: 0; padding-left: 20px; color: #333333; font-size: 15px; line-height: 2;">{items_html}</ul>'
            "</section></section>"
        )
    return (
...
def render_paragraph_block(block: dict[str, Any], *, template_name: str = "") -> str:
    ...
    if is_campus_party_template(template_name):
        return (
            '<section style="background: #ffd897; padding: 0 16px 14px;">'
            '<section style="padding: 12px 15px 10px; border: 1px solid #cd5029; border-bottom: none; background: #fffdf6;">'
            f"{body}"
            "</section>"
            '<section style="display: flex; align-items: flex-start; margin: 0 0 0 10px;">'
            '<section style="width: 100%; border-top: 1px solid #cd5029; height: 1px;"></section>'
            '<section style="flex-shrink: 0; width: 72px; height: 10px; margin: -5px 0 0; background: #e60012; border-radius: 10px;"></section>'
            "</section></section>"
        )
    ...
    for section in article.get("sections") or []:
        ...
        if intro:
            if is_studio_brief_template(template_name):
                ...
                rendered.append(
                    '<section style="background: #ffd897; padding: 0 16px 14px;">'
                    '<section style="background: #ffffff; border-radius: 10px; padding: 14px 16px; border: 1px solid #f0c35c;">'
                    f'<p style="font-size: 15px; color: #333333; line-height: 2; margin: 0; text-indent: 2em;">{normalize_text(intro, allow_html=True)}</p>'
                    "</section></section>"
                )
            else:
                rendered.append(
...
def render_headline_body(article: dict[str, Any], *, template_name: str = "") -> str:
    ...
    if is_campus_party_template(template_name):
        return "".join(
            f'<p style="font-size: 15px; color: #333333; line-height: 2; margin: 0 0 10px; text-align: justify; text-indent: 2em;">{normalize_text(paragraph, allow_html=True)}</p>'
            for paragraph in paragraphs
        )
    return "".join(
