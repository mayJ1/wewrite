...
def get_draft_article(access_token: str, media_id: str) -> dict:
    """
    Get the first draft article from WeChat by media_id.
    ...
    if not isinstance(article, dict):
        raise ValueError(f"WeChat get_draft: invalid article in draft {media_id}")
    return article


def get_draft(access_token: str, media_id: str) -> str:
    """Get the HTML content of the first article in a WeChat draft."""
    return str(get_draft_article(access_token, media_id).get("content") or "")
